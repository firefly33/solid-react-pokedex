...
const PokemonCard = ({ pokemon, click }: PokemonCardProps) => {
    ...
    return (
      <div className="relative px-4 py-2 rounded-2xl overflow-hidden" onClick={() => click(id)}>
        <div className={`absolute inset-0 ${bgColor}`}></div>
        <div className="absolute inset-0">
          <div className='absolute inset-0 bg-no-repeat bg-right-bottom flex flex-col-reverse'  style={{ backgroundImage: `url(${Vector})`, backgroundSize: '8rem 8rem'}}></div>
        </div>
        ...
        <div className="relative z-10 ">
          <div className={`w-auto flex flex-col justify-between`}>
            <div className='flex justify-between gap-2'>
              ...
              <span className='flex justify-start text-black text-opacity-20  font-semibold text-2xl uppercase'>#{paddPokemonNumber(pokemon.id)}</span>
            </div>
    ...
    return (
        <div className={`w-auto flex flex-col justify-between px-4 py-2 rounded-2xl ${bgColor}`} style={{ backgroundImage: 'url("../../../assets/Vector.svg")'}}>
